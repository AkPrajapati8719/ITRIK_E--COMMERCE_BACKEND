...
def home(request):
    """
    Serve index.html locally in development.
    In production (Render) return API info.
    """
    if settings.DEBUG:
...
def serve_html_file(request, filename):
    ...
    return JsonResponse(
        {"error": "API Mode: Use the Vercel frontend to access pages."},
        status=404
    )
...
if settings.DEBUG:
    ...
    # Catch-all HTML routes (must stay at bottom)
    urlpatterns += [
        path(
            "delivery_panel.html",
            serve_html_file,
            kwargs={"filename": "delivery_panel"}
        ),
